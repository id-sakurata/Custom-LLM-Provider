...
import { ToolFlavor } from './types';

interface OAIToolDefinition {
    type: 'function';
    function: { name: string; description: string; parameters: unknown };
}

interface OAIFunctionDefinition {
    name: string;
    description: string;
    parameters: unknown;
}
...
export class ToolAdapter {
     ...
     */
    public static translate(tools: readonly vscode.LanguageModelChatTool[], flavor: ToolFlavor): TranslatedTools {
     ...
     */
    public static detectToolCallInText(text: string): { name: string; args: Record<string, unknown> } | null {
        try {
            ...
            if (typeof record.function === 'string' && record.function.length > 0 && record.parameters !== undefined) {
                return { name: record.function, args: record.parameters as Record<string, unknown> };
            }
            if (typeof record.tool === 'string' && record.tool.length > 0 && record.parameters !== undefined) {
                return { name: record.tool, args: record.parameters as Record<string, unknown> };
            }
        } catch {
